...
#define VKIL_SHM_FLAG    IPC_CREAT | 0666
#define VKIL_SHM_ID      shmget(VKIL_SHM_KEY, VKIL_SHM_SIZE, VKIL_SHM_FLAG)

/*
- modified from neverMind's answer on https://stackoverflow.com/questions/2143404/delete-all-system-v-shared-memory-and-semaphores-on-unix-like-systems
- run the following script to erase the shm in case of testing
- 0x0000007b is the key value in hex

    #!/bin/bash
    shmid=`ipcs -m | grep 0x0000007b | cut -f2 -d" "`
    ipcrm -m $shmid;
*/

/*
To do or/and re-evaluate:
    - concurrency control since multiple processes will be accessing/modifying the shared table
        - locks (?)
    - underlying structure for the session table
        - lookups could be optimized (?)
        - table size
    - finished sessions
        - when to remove entry from table (right now 'pid active' check every time vkil_get_session_table() is called)
    - shm key
    - card selection
        - refer to Valkyrie doc
        - probably require arguments
        - should it be in a seperate file (if complex)
*/
